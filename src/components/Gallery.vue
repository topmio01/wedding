...
function getEffectiveConfig() {
  // Tune parameters for mobile for performance/usability
  return {
    items: props.items,
    bend: isMobile.value ? Math.min(props.bend, 1) : props.bend,
    textColor: props.textColor,
    borderRadius: isMobile.value ? Math.min(props.borderRadius, 0.04) : props.borderRadius,
    font: isMobile.value ? 'bold 22px Figtree' : props.font,
    scrollSpeed: isMobile.value ? 1 : props.scrollSpeed,
...
onMounted(() => {
  ...
  app = new App(containerRef.value, getEffectiveConfig());
});
...
watch(
  ...
  () => {
    if (app) app.destroy();
    if (containerRef.value) app = new App(containerRef.value, getEffectiveConfig());
  },
  { deep: true }
);
</script>
